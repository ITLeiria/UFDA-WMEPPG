...
class HRSensorDetailsViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, CBPeripheralDelegate, NSFetchedResultsControllerDelegate {
    ...
    @objc func updateCounting(){
        if countTimesReloadTable >= 3 && !isBLEUpdating {
    ...
    func createBLEDevicesCell(tableView: UITableView, indexPath: IndexPath) -> UITableViewCell {
        ...
        if (manualUpdatedTable) {
            countTimesReloadTable = 0
            //            countNotReading += 1
            //            if arrayDataHR_results.count == indexPath.count && arrayDataPO_results.count == indexPath.count {
            
            //                    if (!isBLEUpdating) {
            //                if (isDifferentValue[indexPath.row]){}
            //                else{
            if !(didUpdateValueHR[indexPath.row]) && !(didUpdateValueSpO2[indexPath.row]) {
                
                if let actualHR = Int((arrayDataHR_results[indexPath.row]?[0])!) {
                    if let actualSpO2 = Float(arrayDataPO_results[indexPath.row]![0]) {
                        if lastHR[indexPath.row] == actualHR && lastSpO2[indexPath.row] == actualSpO2 {
                            if (countNotReading >= 2) {
                                connectDeviceCell.viewMeasurementPart.backgroundColor = UIColor.customWarmGray()
                            }
                        }
                    }
                }
                
                if lastHR[indexPath.row] == Int(arrayDataHR_results[indexPath.row]![0]) && lastSpO2[indexPath.row] == Float(arrayDataPO_results[indexPath.row]![0]) {
                    if (countNotReading >= 2) {
                        connectDeviceCell.viewMeasurementPart.backgroundColor = UIColor.customWarmGray()
                    }
                }
...
//                            connectDeviceCell.viewChartIR.chartDescription?.text = "IR Raw Chart"
...
//                            connectDeviceCell.viewChartIR.setVisibleXRange(minXRange: 20, maxXRange: 100)
...
//                            connectDeviceCell.viewChartIR.setYAxisMaxWidth(YAxis.AxisDependency.right, width: CGFloat(dataChartIR.getYMax()))
                            ...
                            if (!(heartRateCD.body_location.isEmpty) && (!heartRateCD.pulse_oximeter.isZero) && (heartRateCD.heart_beat != 0)) {
                                ...
                                if dictX["heart_beat"] != nil && dictX["pulse_oximeter"] != nil {
                                    //                if (!(heartRateCD.ir_value.isEmpty) && !(heartRateCD.pulse_oximeter.isZero)){
                                    let ir_value = arrayRawDataIR_oneBeat
                                    //                    print("ir value \(arrayRawDataIR_oneBeat)")
                                    ...
                                    // mudar aqui para colocar os items
                                    if sharedSnapshot != nil {
                                        let setHealthData = HealthData(snapshot: sharedSnapshot)
                                        //                        var setItem = Item(snapshot: sharedSnapshot)
                                        //                        var oxi = Item.epa(uid: id, body_location: dictX["body_location"] as? String, heart_beat: dictX["heart_beat"] as? Int, ir_value: dictX["ir_value"] as? [String], measurement_date: dictX["measurement_date"] as? Int, pulse_oximeter: dictX["pulse_oximeter"] as? Float)
                                        //                        setItem = oxi
                        ...
                        if arrayDataHR_results.count == indexPath.count && arrayDataPO_results.count == indexPath.count {
                            ...
                            if !(didUpdateValueHR[indexPath.row]) && !(didUpdateValueSpO2[indexPath.row]) {
                                if lastHR[indexPath.row] == Int(arrayDataHR_results[indexPath.row]![0]) && lastSpO2[indexPath.row] == Float(arrayDataPO_results[indexPath.row]![0]) {
                                    //                                        isDifferentValue[indexPath.row] = false
                                    
                                    if (countNotReading > 3) {
                                        connectDeviceCell.viewMeasurementPart.backgroundColor = UIColor.customWarmGray()
                                    }
                                }
                            }
                ...
                //            countNotReading += 1
                //            if (countNotReading > 2) {
                //                connectDeviceCell.viewMeasurementPart.backgroundColor = UIColor.customDarkGrey()
                //            }
                //            didUpdateValueHR[indexPath.row] = false
                //        }
                
                //        if arrayDataHR_results.count == indexPath.count {
                //            if didUpdateValueHR[indexPath.row] {
                //
                ////                connectDeviceCell.viewMeasurementPart.backgroundColor = UIColor.customRedOpacity85()
                //                countNotReading = 0
        ...
        {
            //        let dataHR = HeartRateData()
            //        let heartRate = HeartRate(entity: HeartRate.entity(), insertInto: context)
                ...
                if (characteristic.uuid == BluetoothGATT.BLECharacteristics.irRawData.UUID)
                {
                    for i in 0..<arrayCharIR.count {
                        if characteristic.hashValue == arrayCharIR[i] {
                            ...
                            //                        print("arrayRawDataIR_results = \(arrayRawDataIR_results)")
                            //                        arrayRawDataIR_results.updateValue(arrayRawDataIR , forKey: i)
                            
                            // aqui nao estava comentado para funcionar
                            if i == 0 {
        ...
        @objc func updateBodyLocation(notification: NSNotification){
            ...
            // MARK: - TODO
            // Chamar uma funcao para alterar a imagem do local onde o sensor esta inserido.
            // Aqui ja tenho qual sensor eh e local
            // ele esta sobrescrevendo o userdefaults, ou seja, o parametro (forKey) tem que ser
            // o index da celula + "newBodyLocation" - tem que haver alguma diferenciacao
        ...
        func buttonSensorBodyLocationTapped(_ button: UIButton) {
            ...
            //        self.performSegue(withIdentifier: "segueUpdateBodyLocation", sender: self)
            
            
            //        showBodySensorLocationSelection()
            //        print(newBodyLocation)
